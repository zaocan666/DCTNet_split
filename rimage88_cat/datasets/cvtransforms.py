...
import datasets.cvfunctional as F
...
class TransformUpscaledDCT(object):
    ...
    def __call__(self, img):
        y, cbcr = img[0], img[1]
        dct_y, _, _ = F.transform_dct(y, self.jpeg_encoder) #size: y/8
        _, dct_cb, dct_cr = F.transform_dct(cbcr, self.jpeg_encoder) #size: cbcr/16
...
class SubsetDCT_part(object):
    def __init__(self, channels_low=6, channels_high=24):
        self.subset_channel_index = subset_channel_index
        self.subset_low_y =  self.subset_channel_index[channels_low][0]
        self.subset_low_cb = self.subset_channel_index[channels_low][1]
        self.subset_low_cr = self.subset_channel_index[channels_low][2]

        self.subset_high_y =  list(set(self.subset_channel_index[channels_high][0])-set(self.subset_channel_index[channels_low][0]))
        self.subset_high_cb = list(set(self.subset_channel_index[channels_high][1])-set(self.subset_channel_index[channels_low][1]))
        self.subset_high_cr = list(set(self.subset_channel_index[channels_high][2])-set(self.subset_channel_index[channels_low][2]))

    def __call__(self, tensor):
        dct_y, dct_cb, dct_cr = tensor[0], tensor[1], tensor[2]
        dct_low_y, dct_low_cb, dct_low_cr = dct_y[self.subset_low_y], dct_cb[self.subset_low_cb], dct_cr[self.subset_low_cr]
        dct_high_y, dct_high_cb, dct_high_cr = dct_y[self.subset_high_y], dct_cb[self.subset_high_cb], dct_cr[self.subset_high_cr]
        return dct_low_y, dct_low_cb, dct_low_cr, dct_high_y, dct_high_cb, dct_high_cr
...
class Aggregate_part(object):
    def __call__(self, img):
        dct_low_y, dct_low_cb, dct_low_cr = img[0], img[1], img[2]
        dct_high_y, dct_high_cb, dct_high_cr = img[3], img[4], img[5]
        dct_low_y = torch.cat((dct_low_y, dct_low_cb, dct_low_cr), dim=0)
        dct_high_y = torch.cat((dct_high_y, dct_high_cb, dct_high_cr), dim=0)
        return dct_low_y, dct_high_y
...
class NormalizeDCT_part(object):
    """Normalize a tensor image with mean and standard deviation.
    Given mean: ``(M1,...,Mn)`` and std: ``(S1,..,Sn)`` for ``n`` channels, this transform
    will normalize each channel of the input ``torch.*Tensor`` i.e.
    ``input[channel] = (input[channel] - mean[channel]) / std[channel]``
    ...
    Args:
        ...
        std (sequence): Sequence of standard deviations for each channel.
    """
    def __init__(self, y_mean, y_std, cb_mean=None, cb_std=None, cr_mean=None, cr_std=None, channels_low=None, channels_high=None):
        self.y_mean,  self.y_std = y_mean, y_std
        self.cb_mean, self.cb_std = cb_mean, cb_std
        self.cr_mean, self.cr_std = cr_mean, cr_std
        ...
        self.subset_low_y  = self.subset_channel_index[channels_low][0]
        self.subset_low_cb = self.subset_channel_index[channels_low][1]
        self.subset_low_cb = [64+c for c in self.subset_low_cb]
        self.subset_low_cr = self.subset_channel_index[channels_low][2]
        self.subset_low_cr = [128+c for c in self.subset_low_cr]
        self.subset_low = sorted(self.subset_low_y + self.subset_low_cb + self.subset_low_cr)
        self.mean_low_y, self.std_low_y = [y_mean[i] for i in self.subset_low], [y_std[i] for i in self.subset_low]

        self.subset_high_y  = list(set(self.subset_channel_index[channels_high][0])-set(self.subset_channel_index[channels_low][0]))
        self.subset_high_cb = list(set(self.subset_channel_index[channels_high][1])-set(self.subset_channel_index[channels_low][1]))
        self.subset_high_cb = [64+c for c in self.subset_high_cb]
        self.subset_high_cr = list(set(self.subset_channel_index[channels_high][2])-set(self.subset_channel_index[channels_low][2]))
        self.subset_high_cr = [128+c for c in self.subset_high_cr]
        self.subset_high = sorted(self.subset_high_y + self.subset_high_cb + self.subset_high_cr)
        self.mean_high_y, self.std_high_y = [y_mean[i] for i in self.subset_high], [y_std[i] for i in self.subset_high]
    ...
    def __call__(self, tensor):
        ...
        Returns:
            Tensor: Normalized Tensor image.
        """
        y_low, y_high = tensor[0], tensor[1]
        y_low = F.normalize(y_low, self.mean_low_y, self.std_low_y)
        y_high = F.normalize(y_high, self.mean_high_y, self.std_high_y)
        return y_low, y_high
