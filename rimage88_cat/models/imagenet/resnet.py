...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, first_stride, cat_pos, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, reduce_factor=1, cat_factor=0):
        ...
        self.inplanes = int(64 * reduce_factor)
        self.dilation = 1
        ...
        if [0,0] in cat_pos:
            self.layer1 = self._make_layer(block, int(64*reduce_factor), layers[0], cat_factor=cat_factor)
        else:
            self.layer1 = self._make_layer(block, int(64*reduce_factor), layers[0], cat_factor=0)

        if 0 in cat_pos:
            self.inplanes = int(self.inplanes*(1+cat_factor))
        # Change the stride from 2 to 1 to match the input size
        self.layer2 = self._make_layer(block, int(128*reduce_factor), layers[1], stride=first_stride,
                                       dilate=replace_stride_with_dilation[0])
        if 1 in cat_pos:
            self.inplanes = int(self.inplanes*(1+cat_factor))
        self.layer3 = self._make_layer(block, int(256*reduce_factor), layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        if 2 in cat_pos:
            self.inplanes = int(self.inplanes*(1+cat_factor))
        self.layer4 = self._make_layer(block, int(512*reduce_factor), layers[3], stride=2,
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, cat_factor=0):
...
def _resnet(arch, block, layers, pretrained, progress, classes=1000,**kwargs):
    model = ResNet(block=block, layers=layers, num_classes=classes, **kwargs)
    if pretrained:
...
def resnet50(first_stride, pretrained=False, reduce_factor=1, progress=True, classes=1000, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    model = _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress, classes, reduce_factor=reduce_factor, first_stride=first_stride,
                   **kwargs)
...
class ResNetDCT_Upscaled_Static(nn.Module):
    def __init__(self, first_stride, cat_pos, cat_factor=0, channels=0, reduce_factor=1, pretrained=True, input_gate=False, classes=1000):
        super(ResNetDCT_Upscaled_Static, self).__init__()

        self.input_gate = input_gate

        model = resnet50(pretrained=pretrained, classes=classes, reduce_factor=reduce_factor, first_stride=first_stride, cat_factor=cat_factor, cat_pos=cat_pos)

        self.model = nn.Sequential(*list(model.children())[4:-1]) #same with paper
        self.fc = list(model.children())[-1]
        self.relu = nn.ReLU(inplace=True)
        
        if channels == 0 or channels == 192 or channels>(64*reduce_factor):
            out_ch = self.model[0][0].conv1.out_channels #64
        ...
        elif channels <= (64*reduce_factor):
