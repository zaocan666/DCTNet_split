...
args.subset_low = str(all_checkpoint['module.low_model_feature.0.0.conv1.weight'].shape[1])
args.subset_high = str(all_checkpoint['module.high_model_feature.0.0.conv1.weight'].shape[1]+int(args.subset_low))
...
def main():
    ...
    model = Part_model(low_checkpoint=None, subset_low=args.subset_low, subset_high=args.subset_high, 
        low_reduce_factor=low_reduce_factor, num_classes=100, bound=args.bound)

    for p in model.low_model_feature.parameters():
        p.requires_grad = False
    for p in model.high_model_feature.parameters():
        p.requires_grad = False
    for p in model.fc_fuse.parameters():
        p.requires_grad = False
    ...
    model.load_state_dict(all_checkpoint)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    model.train()
    model.module.low_model_feature.eval()
    ...
    for batch_idx, (image_low, image_high, target) in enumerate(train_loader):
        ...
        feature_low = feature_low.detach()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (image_low, image_high, target) in enumerate(val_loader):
            ...
            if fc_eval:
                ...
                histogram_correct, value_acc, correct_num, bound_num = cal_his_acc([[args.bound, 1]], output_softmax.cpu().data.numpy(), target.cpu().data.numpy())
