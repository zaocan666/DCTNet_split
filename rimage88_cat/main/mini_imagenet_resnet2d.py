...
model_name = 'fre_resnet3d_mini_arch_resnet_' + str(args.arch) + '_sgd_lr_' + str(args.lr) + '_bz_' + str(args.train_batch) + '_scheduler_' + str(args.lr_scheduler) + '_epoch_' + str(args.epoch) \
 + '_subset_' + str(args.subset)
print(model_name)
...
def main():
    ...
    model = ResNetDCT_Upscaled_Static(channels=int(args.subset), pretrained=args.pretrained, classes=100, reduce_factor=1, first_stride=2)
