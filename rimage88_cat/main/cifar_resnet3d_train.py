...
from models.imagenet.resnet3d import generate_model
...
parser.add_argument('--train_batch', default=512, type=int, metavar='N',
...
parser.add_argument('--subset', default='48', type=str, help='subset of y, cb, cr')
#Device options
...
model_name = 'fre_resnet3d' + '_optim_SGD' '_lr_' + str(args.lr) + '_bz_' + str(args.train_batch) + '_widen_factor_' + str(args.w)
...
def main():
    ...
    model = generate_model(50, n_classes=10, n_input_channels=3, widen_factor=args.w)
    # define loss function (criterion) and optimizer
    criterion = nn.CrossEntropyLoss().cuda()
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    # optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, weight_decay=4e-5, nesterov=False)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for batch_idx, (image, target) in enumerate(train_loader):
        ...
        # non_blocking=True
        # image, target = image.cuda(), target.cuda()  # ([200, 24, 56, 56])  ([200])
        data = image.reshape(image.size(0),3,int(args.subset)//3,image.shape[2],image.shape[3])
        ...
        # plot progress
        record  = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}'.format(
                    batch=batch_idx + 1,
...
def test(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (image, target) in enumerate(val_loader):
            ...
            data = image.reshape(args.test_batch,3,int(args.subset)//3,image.shape[2],image.shape[3])
            data, target = data.cuda(), target.cuda()
