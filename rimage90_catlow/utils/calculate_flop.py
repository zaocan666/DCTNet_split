...
model = Part_model(low_checkpoint=None, subset_low=subset_low, subset_high=subset_high, 
        low_reduce_factor=low_reduce_factor, num_classes=100, bound=1, first_stride=2, cat_pos=[])

if mode == 'normal':
    # compute output
    summary(model, torch.rand([1, int(subset_low), 56, 56]), torch.rand([1, int(subset_high)-int(subset_low), 56, 56]), 1)
elif mode == 'low':
    # part_model = nn.Sequential(
    #     model.low_input_layer,
    #     model.low_model_feature,
    #     nn.Flatten(),
    #     model.low_model_fc,
    # )
    # summary(part_model, torch.rand([1, int(subset_low), 56, 56]))
    summary(model, torch.rand([1, int(subset_low), 56, 56]), torch.rand([1, int(subset_high)-int(subset_low), 56, 56]), 2)
# elif mode == 'high':
#     part_model = nn.Sequential(
...
#     summary(part_model, torch.rand([1, int(subset_high)-int(subset_low), 56, 56]))
    ...
    summary(model, torch.rand([1, int(subset_low), 56, 56]), torch.rand([1, int(subset_high)-int(subset_low), 56, 56]), 0)
